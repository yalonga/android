package com.ioter.hopeland.uhf.UHF5Base;
...
public abstract class ReaderBase {
	 ...
	 * 设置串口通讯波特率。
	 * @param btReadId			读写器地址(0xFF公用地址)
	 * @param nIndexBaudrate	波特率(0x03: 38400bps, 0x04:115200 bps)
	 * @return	成功 :0, 失败:-1
	 */
	...
	/**
	 * 设置读写器地址。
	 * @param btReadId		原读写器地址(0xFF公用地址)
	 * @param btNewReadId	新读写器地址,取值范围0-254(0x00–0xFE)
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 设置读写器工作天线。
	 * @param btReadId		读写器地址(0xFF公用地址)
	 * @param btWorkAntenna	天线号(0x00:天线1, 0x01:天线2, 0x02:天线3, 0x03:天线4)
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 设置读写器射频输出功率(方式1)。
	 * <br>★此命令耗时将超过100mS。
	 * <br>★如果需要动态改变射频输出功率，请使用CmdCode_set_temporary_output_power命令，否则将会影响Flash的使用寿命。
	 * @param btReadId		读写器地址(0xFF公用地址)
	 * @param btOutputPower	RF输出功率,取值范围0-33(0x00–0x21), 单位dBm
	 * @return	成功 :0, 失败:-1
	 */
	...
	/**
	 * 设置读写器射频输出功率(方式2)。187 1851 1872
	 * <br>★此命令耗时将超过100mS。
	 * <br>★如果需要动态改变射频输出功率，请使用CmdCode_set_temporary_output_power命令，否则将会影响Flash的使用寿命。
	 * @param btReadId		读写器地址(0xFF公用地址)
	 * @param btPower1	RF天线1输出功率,取值范围0-33(0x00–0x21), 单位dBm
	 * @param btPower2	RF天线2输出功率,取值范围0-33(0x00–0x21), 单位dBm
	 * @param btPower3	RF天线3输出功率,取值范围0-33(0x00–0x21), 单位dBm
	 * @param btPower4	RF天线4输出功率,取值范围0-33(0x00–0x21), 单位dBm
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * @param btReadId		读写器地址(0xFF公用地址)
	 * @param btRegion		射频规范(0x01:FCC, 0x02:ETSI, 0x03:CHN)
	 * @param btStartRegion	频率起始点,
	 * @param btEndRegion	频率结束点,可以在射频规范的频率范围内再设置跳频的范围. 参数的设置规则为: 1,起始频率与结束频率不能超过射频规范的范围. 2,起始频率必须低于结束频率. 3,起始频率等于结束频率则定频发射
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 设置读写器工作频率范围(用户自定义频谱)。
	 * @param btReadId			读写器地址(0xFF公用地址)
	 * @param btFreqInterval	频点间隔, 频点间隔 = btFreqInterval x 10KHz
	 * @param btChannelQuantity	频点数量, 包含起始频率的频点数量, 1为以起始频率定频发射. 此参数必须大于0
	 * @param nStartFreq		起始频率, 单位为KHz. 16进制数高位在前. 例如915000KHz则发送0x0D 0xF6 0x38
	 * @return	成功 :0, 失败:-1
	 */
	...
	/**
	 * 设置蜂鸣器状态。
	 * <br>★读到一张标签后蜂鸣器鸣响，会占用大量处理器时间，若此选项打开，将会明显影响到读多标签(防冲突算法)的性能，此选项应作为测试功能选用。
	 * @param btReadId	读写器地址(0xFF公用地址)
	 * @param btMode	操作标签时蜂鸣器状态(0x00:安静, 0x01:每次盘存后鸣响, 0x02:每读到一张标签鸣响)
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 设置GPIO电平。
	 * @param btReadId		读写器地址(0xFF公用地址)
	 * @param btChooseGpio	GPIO口选择(0x03:设置GPIO3, 0x04:设置GPIO4)
	 * @param btGpioValue	GPIO设置值(0x00:低电平, 0x01:高电平)
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 设置天线连接检测器状态。
	 * @param btReadId			读写器地址(0xFF公用地址)
	 * @param btDetectorStatus	状态(0x00:关闭天线连接检测, 其他值:天线连接检测的灵敏度(端口回波损耗值),单位dB. 值越大对端口的阻抗匹配要求越高
	 * @return	成功 :0, 失败:-1
	 */
	...
	/**
	 * 设置读写器临时射频输出功率。
	 * <br>操作成功后输出功率值将不会被保存在内部的Flash中，重新启动或断电后输出功率将恢复至内部Flash中保存的输出功率值。此命令的操作速度非常快，并且不写Flash，从而不影响Flash的使用寿命，适合需要反复切换射频输出功率的应用。
	 * @param btReadId	读写器地址(0xFF公用地址)
	 * @param btRfPower	RF输出功率, 取值范围20-33(0x14–0x21), 单位dBm
	 * @return	成功 :0, 失败:-1
	 */
	...
	/**
	 * 设置读写器识别码。
	 * <br>操作成功后12字节的读写器识别字符串将会保存在内部的Flash中，断电后不丢失。
	 * @param btReadId		读写器地址(0xFF公用地址)
	 * @param btAryIdentifier	12字节的读写器识别字符
	 * @return	成功 :0, 失败:-1
	 */
	...
	/**
	 * 设置射频链路的通讯速率。
	 * <br>操作成功后读写器会重新启动，配置保存在内部的Flash中，断电后不丢失。
	 * @param btReadId	读写器地址(0xFF公用地址)
	 * @param btProfile	射频链路的通讯速率(0xD0:Tari 25uS,FM0 40KHz, 0xD1:Tari 25uS,Miller 4 250KHz, 0xD2:Tari 25uS,Miller 4 300KHz, 0xD3:Tari 6.25uS,FM0 400KHz)
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 测量天线端口的回波损耗。
	 * @param btReadId		读写器地址(0xFF公用地址)
	 * @param btFrequency	频率参数, 系统将获取此频点当前工作天线端口的回波损耗值
	 * @return	成功 :0, 失败:-1
	 */
	...
	/**
	 * 盘存标签。
	 * <br>读写器收到此命令后，进行多标签识别操作。标签数据存入读写器缓存区。
	 * <br>注意：
	 * <br>★将参数设置成255(0xFF)时，将启动专为读少量标签设计的算法。对于少量标的应用来说，效率更高，反应更灵敏，但此参数不适合同时读取大量标签的应用。
	 * @param btReadId	读写器地址(0xFF公用地址)
	 * @param btRepeat	盘存过程重复的次数, Repeat=0xFF则此轮盘存时间为最短时间. 如果射频区域内只有一张标签,则此轮的盘存约耗时为30-50mS. 一般在四通道机器上快速轮询多个天线时使用此参数值
	 * @return	成功 :0, 失败:-1
	 */
	...
	public final int readTag(byte btReadId, byte btMemBank, byte btWordAdd,
		...
		int nResult = sendMessage(btReadId, btCmd, btAryData);
		Log.d("readTag", String.valueOf(btReadId)+" "+ String.valueOf(btCmd)+" "+
				String.valueOf(btMemBank)+" "+ String.valueOf(btWordAdd)+" "+ String.valueOf(btWordCnt));
		return nResult;
	}
	 ...
	 * @param btReadId	读写器地址(0xFF公用地址)
	 * @param btEpcLen	EPC长度
	 * @param btAryEpc	EPC号, 由EpcLen个字节组成
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * <br>注意：
	 * <br>★由于硬件为双CPU架构，主CPU负责轮询标签，副CPU负责数据管理。轮询标签和发送数据并行，互不占用对方的时间，因此串口的数据传输不影响读写器工作的效率。
	 * <br>★此命令读取大量标签时，效率没有CmdCode_real_time_inventory命令高。
	 * @param btReadId		读写器地址(0xFF公用地址)
	 * @param btA			首先轮询的天线(00–03), 天线号大于三则表示不轮询
	 * @param btStayA		天线重复轮询的次数
	 * @param btB			第二个轮询的天线(00–03), 天线号大于三则表示不轮询
	 * @param btStayB		天线重复轮询的次数
	 * @param btC			第三个轮询的天线(00–03), 天线号大于三则表示不轮询
	 * @param btStayC		天线重复轮询的次数
	 * @param btD			第四个轮询的天线(00–03), 天线号大于三则表示不轮询
	 * @param btStayD		天线重复轮询的次数
	 * @param btInterval	天线间的休息时间. 单位是mS. 休息时无射频输出,可降低功耗
	 * @param btRepeat		重复以上天线切换顺序次数
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 自定义session和target盘存。
	 * @param btReadId	读写器地址(0xFF公用地址)
	 * @param btSession	指定盘存的session
	 * @param btTarget	指定盘存的Inventoried Flag(0x00:A, 0x01:B)
	 * @param btRepeat	盘存过程重复的次数
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 设置Monza标签快速读TID。
	 * <br>注意：
	 * <br>★此功能仅对Impinj Monza 标签的部分型号有效。
	 * <br>★此功能在识别EPC的同时识别TID，因此大大提高了读TID的效率。
	 * <br>★打开此功能后，特定型号的标签会在盘存的过程中将TID打包到EPC中。因此，标签的PC会被修改，原来的PC+EPC变为：修改后的PC+EPC+(EPC的CRC)+TID。
	 * <br>★如果在识别TID的过程中出现错误，则上传原来的PC+EPC。
	 * <br>★如不需要此功能请将其关闭，避免不必要的时间消耗。
	 * <br>★此命令不保存至内部的Flash中，重启后将回复至Flash保存的值。
	 * @param btReadId	读写器地址(0xFF公用地址)
	 * @param blnOpen	是否打开FastTID
	 * @param blnSave	是否保存至内部FLASH
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 读18000-6B标签。
	 * @param btReadId	读写器地址(0xFF公用地址)
	 * @param btAryUID	被操作标签的UID, 8 bytes
	 * @param btWordAdd	要读取的数据首地址
	 * @param btWordCnt	要读取的数据长度
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * @param btReadId		读写器地址(0xFF公用地址)
	 * @param btAryUID		被操作标签的UID, 8 bytes
	 * @param btWordAdd		写入数据的首地址
	 * @param btWordCnt		写入数据的长度
	 * @param btAryBuffer	写入的数据
	 * @return	成功 :0, 失败:-1
	 */
	 ...
	 * 提取标签数据并删除缓存。
	 * <br>注意：
	 * <br>★命令完成后，缓存中的数据并不丢失，可以多次提取。
	 * <br>★若再次运行CmdCode_inventory 命令，则盘存到的标签将累计存入缓存。
	 * <br>★若再次运行其他的18000-6C命令，缓存中的数据将被清空。
	 * @param btReadId	读写器地址(0xFF公用地址)
	 * @return	成功 :0, 失败:-1
	 */
