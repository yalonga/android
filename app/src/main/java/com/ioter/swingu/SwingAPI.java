...
public class SwingAPI
{
    ...
    public synchronized void manageConnectedSocket(BluetoothSocket socket, BluetoothDevice device)
    {
        ...
        bundle.putString(MainActivity.SCANNER_ADDR, device.getAddress());
        msg.setData(bundle);
    ...
    private void write(byte[] out)
    {
        ...
        String debug = new String(out);
        Log.d("SwingAPI", debug);
        // Perform the write unsynchronized
        r.write(out);
    }
    ...
    // wsjang@170321
    public void swing_set_keymask(byte u, byte d, byte s, byte f)
    {
        String cmdString = String.format(">zs%d%d%d%d\r", u, d, s, f);

        byte[] pkt = cmdString.getBytes();
        write(pkt);
    }

    public void swing_vibrate()
    {
        String cmdString = ">zv\r";
    ...
    public void TagListAdd(String UID)
    {
        String cmdString = ">x t ";
        cmdString += UID;
    ...
    public boolean TagListAddSync(String UID)
    {
        String cmdString = ">x t ";
        cmdString += UID;
        byte[] ack = null;
        ...
        {
            return true;
        } else if (acktostring.contains(">E00004"))
        {
            return true;
        }
        return false;
...
*/
    ...
    public boolean setSession(int Session)
    {
        ...
        String cmdString = ">x 4 ";
        cmdString += String.valueOf(Session);
        cmdString += "\r";
    ...
    private void connectionFailed()
    {
        ...
        Bundle bundle = new Bundle();
        bundle.putString(MainActivity.TOAST, "Scanner connection fail");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    ...
    private class ConnectedThread extends Thread
    {
        ...
        private void parse(byte[] buffer, int length)
        {
            ...
            Log.i("dsm362", String.format("rx packet = %s", str_ack));//epc值
            ...
            try
            {
                ...
                switch (cmd)
                {
                    ...
                    case 'A':
                        ...
                        {
                            mHandler.obtainMessage(MainActivity.MESSAGE_FOUND).sendToTarget();
                        } else if (buffer[2] == '4')
                        {
                            mHandler.obtainMessage(MainActivity.MESSAGE_SEARCH_FINISH).sendToTarget();
                        } else if (buffer[2] == ' ')
                        {
                            mHandler.obtainMessage(MainActivity.MESSAGE_MENU_ENABLE, datalength, -1, data).sendToTarget();
                        ...
                        break;
                    case 'E':
                        mHandler.obtainMessage(MainActivity.MESSAGE_ERROR, datalength, -1, data).sendToTarget();
                        sync_update(buffer, length);
                        break;
                    case 'T':    // 'T', tag report mode
                        mHandler.obtainMessage(MainActivity.MESSAGE_TAG, datalength, -1, data).sendToTarget();
                        break;
                    case 'J':    // barcode mode
                        mHandler.obtainMessage(MainActivity.MESSAGE_TAG, datalength, -1, data).sendToTarget();
                        break;
                    case 'R':    // 'R', tag memory report mode
                        mHandler.obtainMessage(MainActivity.MESSAGE_READ, datalength, -1, data).sendToTarget();
                        sync_update(buffer, length);
                        break;
                    case 'c':    // 'c', get continuous mode
                        Log.d("dsm362", String.format("continuous mode = %s", new String(data, 0, datalength)));
                        mHandler.obtainMessage(MainActivity.MESSAGE_CONTINUOUS, datalength, -1, data).sendToTarget();
                        sync_update(buffer, length);
                        break;
                    case 's':    // 's', get buzzer volume
                        if (buffer[2] == 't') break;
                        Log.d("dsm362", String.format("buzzer volume = %s", new String(data, 0, datalength)));
                        mHandler.obtainMessage(MainActivity.MESSAGE_VOLUME, datalength, -1, data).sendToTarget();
                        sync_update(buffer, length);
                        break;
                    case 'b':    // 'b', get report mode, 0: trigger, 1: all tag
                        Log.d("dsm362", String.format("report mode = %s", new String(data, 0, datalength)));
                        mHandler.obtainMessage(MainActivity.MESSAGE_REPORT, datalength, -1, data).sendToTarget();
                        sync_update(buffer, length);
                        break;
                    case 'p':    // 'p', get power, 0, 1, 2 mode
                        Log.d("dsm362", String.format("rf atten = %s", new String(data, 0, datalength)));
                        mHandler.obtainMessage(MainActivity.MESSAGE_RFPOWER, datalength, -1, data).sendToTarget();
                        sync_update(buffer, length);
                        break;
                    case 'k':    // 'p', get power, 0, 1, 2 mode
                        Log.d("dsm362", String.format("rf th = %s", new String(data, 0, datalength)));
                        mHandler.obtainMessage(MainActivity.MESSAGE_THRESHOLD, datalength, -1, data).sendToTarget();
                        sync_update(buffer, length);
                        break;
                    case 'u':    // 'p', get power, 0, 1, 2 mode
                        Log.d("dsm362", String.format("down unit = %s", new String(data, 0, datalength)));
                        mHandler.obtainMessage(MainActivity.MESSAGE_UNIT, datalength, -1, data).sendToTarget();
                        sync_update(buffer, length);
                        break;
                    case 'v':
                        mHandler.obtainMessage(MainActivity.MESSAGE_BATTERY, datalength, -1, data).sendToTarget();
                        break;
                    case 'm':
                        ...
                        break;
                    case 'n':
                        mHandler.obtainMessage(MainActivity.MESSAGE_TAG_COUNT, datalength, -1, data).sendToTarget();
                        sync_update(buffer, length);
                        break;
                    case 'f':
                        ...
                        if (temp2.contains("3"))
                        {
                            mHandler.obtainMessage(MainActivity.MESSAGE_CLEAR, datalength, -1, data).sendToTarget();
                        } else
                        {
                        }
                    case 'G':
                        String temp1 = new String(data, 0, datalength).trim();
                        mHandler.obtainMessage(MainActivity.MESSAGE_FIND, datalength, -1, data).sendToTarget();
                        break;
                    case 'x':
                        mHandler.obtainMessage(MainActivity.MESSAGE_TEMPERATURE, datalength, -1, data).sendToTarget();
                        break;
                    case '1':
                        mHandler.obtainMessage(MainActivity.MESSAGE_LANGUAGE, datalength, -1, data).sendToTarget();
                        break;
                    case '2':
                        mHandler.obtainMessage(MainActivity.MESSAGE_EPC_SIZE, datalength, -1, data).sendToTarget();
                        break;
                    case '4':
                        mHandler.obtainMessage(MainActivity.MESSAGE_SESSION, datalength, -1, data).sendToTarget();
                        break;
